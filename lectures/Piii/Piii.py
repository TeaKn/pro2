...
class GraphException(Exception):
  ...
  """
  def __init__(self):
    super().__init__("graph is empty since it contains no nodes")
...
class Graph:
  ...
  def __str__(self):
    ...
    return "{0:>12s} | '{1:s}'\n{2:>12s} | '{3:s}'\n{4:>12s} | {5:,d} ({6:,d})\n{7:>12s} | {8:,d} ({9:,d})\n{10:>12s} | {11:.4f}\n{12:>12s} | {13:.1f}% ({14:,d})\n{15:>12s} | {16:.1f} sec\n".format('Graph', self.name, 'Type', ('x' if self.is_labeled() else '0') + ('--' if self.is_simple() else '==') + ('y' if self.is_labeled() else '1'), 'Nodes', self.n, self.get_s(), 'Edges', self.m, self.get_l(), 'Degree', self.get_k(), 'Component', 100.0 * max([len(c) for c in C]) / self.n, len(C), 'Time', time() - start)
  ...
  def components(G):
    ...
    N = set(G.get_nodes())
    while N:
      C.append(Graph.component(G, N, random.sample(N, 1)[0]))
    return C
...
# prints out standard information of some real networks
