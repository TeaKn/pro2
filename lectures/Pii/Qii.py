...
def synonym(word):
  ...
  """
  word = word.group()
  res = re.search(r'<a ng-click="handleClick\(\$event\)" class="dictentry" id="[a-zčšž]+"\s*rel="nofollow">[\sa-zčšž]+</a>', requests.get('https://kontekst.io/kontekst/' + word.lower()).text)
  syn = word if res is None else re.sub(r'<.*?>', '', re.sub(r'\s', '', res.group()))
...
def translate(text, target = 'en'):
  ...
  """
  return requests.get('https://translation.googleapis.com/language/translate/v2?q=' + re.sub(r'\s', '+', text) + '&source=sl&target=' + target + '&key=' + GOOGLE_KEY).json()['data']['translations'][0]['translatedText']
...
for city in cities:
  ...
  print("{0:>12s} | '{1:s}, Slovenija'".format('City', city['kraj']))
